...
class Users(Base, ModelBaseMixin):
    ...
    role: Mapped[int] = mapped_column(
        Integer, nullable=False, default=UserRole.PLAYER
    )  # 0: player, 1: coach
...
class Profiles(Base, ModelBaseMixinWithoutDeletedAt):
    ...
    player_dominant: Mapped[DominantHand] = mapped_column(
        Enum(DominantHand, name="dominant_hand_type"), nullable=False
    )
    player_position: Mapped[Position] = mapped_column(
        Enum(Position, name="player_position_type"), nullable=False
    )
...
class Notes(Base, ModelBaseMixin):
    ...
    user: Mapped["Users"] = relationship("Users", back_populates="notes")

    comments: Mapped["Comments"] = relationship("Comments", back_populates="note")
...
class Comments(Base, ModelBaseMixin):
    ...
    note: Mapped["Notes"] = relationship("Notes", back_populates="comments")
...
class TrainingNotes(Base, ModelBaseMixin):
    ...
    training_id: Mapped[UUID] = mapped_column(
        ForeignKey("trainings.id"), nullable=False
    )
    note_id: Mapped[UUID] = mapped_column(ForeignKey("notes.id"), nullable=False)

    count: Mapped[int] = mapped_column(Integer, nullable=False)

    notes: Mapped["Notes"] = relationship("Notes", back_populates="training_notes")
    training: Mapped["Trainings"] = relationship(
        "Trainings", back_populates="training_notes"
    )
